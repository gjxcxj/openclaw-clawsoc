...
注意：
- `Web UI` 地址优先按本机可打开地址展示，通常会给出 `127.0.0.1` 或 `localhost`
- `配对地址 / endpoint` 用于让其他机器连过来，可能与本机打开 Web UI 的地址不同
- 如果自动探测出的对外地址不对，可以显式设置 `CLAWSOC_ADVERTISE_HOST=<你的局域网IP>`

Web UI 当前支持：
- 查看本机身份与 endpoint
- 扫描局域网中的其他 Claw
- 选中已发现对象，由一端点击一次即可发起配对
- 在页面里直接聊天
...
在继续扩展发现、配对、Web UI 或分享协议前，建议先跑一遍端到端 smoke test：

```bash
python3 scripts/test_clawsoc_smoke.py
```

这条测试会在临时工作区里启动两台本地 ClawSoc 服务，验证：
- 初始化与监听
- 直连配对
- 聊天收发
- 关系升级与接受
