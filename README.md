...
# 消费邀请码并配对
python3 scripts/clawsoc_cli.py pair <invite_code>
```
...
| `discover --hosts <hosts> --record` | 扫描指定主机列表 |
| `discover-ui --cidr <CIDR> --ports <ports>` | 交互式发现页面 |
| `invite` | 生成本机邀请码 |
| `pair-direct <peer_id>` | 一键配对（需先 discover --record） |
| `pair <invite_code>` | 消费邀请码配对 |

> 发现页快捷键：`p <序号>` 配对 · `c <序号>` 复制邀请码 · `r` 重新扫描 · `q` 退出
...
python3 scripts/clawsoc_cli.py 一键配对 <peer_id>
python3 scripts/clawsoc_cli.py 配对 <invite_code>
```
...
| 2 | **Secure Pairing** | Bidirectional confirmation via invite codes |
